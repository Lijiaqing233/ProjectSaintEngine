	...
	void *displayLinkContext)
{
	@autoreleasepool
	{
		auto vulkanExample = static_cast<VulkanBase*>(displayLinkContext);
			vulkanExample->displayLinkOutputCb();
	}
...
// SRS - Window resizing already handled by windowResize() in VulkanBase::submitFrame()
...
void VulkanBase::displayLinkOutputCb()
{
#if defined(VK_EXAMPLE_XCODE_GENERATED)
	if (benchmark.active) {
		benchmark.run([=] { render(); }, vulkanDevice->properties);
...
void VulkanBase::mouseDragged(float x, float y)
{
	handleMouseMove(static_cast<uint32_t>(x), static_cast<uint32_t>(y));
}

void VulkanBase::windowWillResize(float x, float y)
{
	resizing = true;
	if (prepared)
	{
...
void VulkanBase::windowDidResize()
{
	resizing = false;
}
#elif defined(_DIRECT2DISPLAY)
#elif defined(VK_USE_PLATFORM_DIRECTFB_EXT)
IDirectFBSurface *VulkanBase::setupWindow()
{
	DFBResult ret;
	int posx = 0, posy = 0;
...
void VulkanBase::handleEvent(const DFBWindowEvent *event)
{
	switch (event->type)
	{
	case DWET_CLOSE:
...
void VulkanBase::pointerMotion(wl_pointer *pointer, uint32_t time, wl_fixed_t sx, wl_fixed_t sy)
{
...
void VulkanBase::setSize(int width, int height)
{
	if (width <= 0 || height <= 0)
		return;
